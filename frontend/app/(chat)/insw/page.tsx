    ...
    const handleNewChat = async () => {
        try {
            const res = await fetchAPI('/chat/sessions', {
                method: 'POST',
                body: JSON.stringify({ chatbot_type: 'INSW', title: 'New Chat' })
            });
            if (res.ok) {
                const data = await res.json();
                router.push(`/insw/${data.session_id}`);
            }
        } catch (e) {
    ...
    return (
        <TooltipProvider delayDuration={300}>
            <div className="flex flex-col items-center justify-center h-full text-muted-foreground p-8 text-center">
                ...
                >
                    Search and query HS Codes, import/export regulations, and trade compliance information.
                </motion.p>
                <motion.div
                    initial={{ y: 10, opacity: 0 }}
                    animate={{ y: 0, opacity: 1 }}
