    ...
    const handleNewChat = async () => {
        try {
            const res = await fetchAPI('/chat/sessions', {
                method: 'POST',
                body: JSON.stringify({ chatbot_type: 'OTHERS', title: 'New Chat' })
            });
            if (res.ok) {
                const data = await res.json();
                router.push(`/others/${data.session_id}`);
            }
        } catch (e) {
            ...
            <div className="flex flex-col items-center justify-center h-full text-muted-foreground p-8 text-center">
                ...
                >
                    <Tooltip>
                        ...
                        <TooltipContent>
                            <p>Internal Information & General Queries</p>
                        </TooltipContent>
                    </Tooltip>
                </motion.div>
                <motion.p
                ...
                >
                    Ask about General EXIM knowledge and internal documents.
                </motion.p>
                <motion.div
                    initial={{ y: 10, opacity: 0 }}
                    animate={{ y: 0, opacity: 1 }}
